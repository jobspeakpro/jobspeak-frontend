...
const PROOF_DIR = path.join(__dirname, 'docs/proofs/2026-01-24_fix_v4/e2e');
...
(async () => {
    log(`🚀 Starting Production E2E Test Suite on ${TARGET_URL}...`);
    ...
    try {
        ...
        await page.fill('[data-testid="affiliate-name"]', 'E2E Test User');
        await page.fill('[data-testid="affiliate-email"]', `e2e_test_${timestamp}@example.com`);
        ...
        await page.fill('[data-testid="affiliate-link"]', 'https://twitter.com/e2etestuser');
        await page.fill('[data-testid="affiliate-strategy"]', 'E2E automated test strategy content for verification');

        // Select PayPal payout
        await page.check('[data-testid="payout-paypal"]');
        await page.fill('[data-testid="affiliate-paypal-email"]', `paypal_e2e_${timestamp}@example.com`);
        ...
        const signupEmail = `e2e_referral_${timestamp}@example.com`;
        const signupPassword = 'E2ETestPassword123!';
        ...
        if (codeInput) {
            ...
            if (refMatch) {
                log(`✅ Referral code extracted: ${refMatch[1]}`);
            }
        ...
        if (videoFile) {
            ...
            const newPath = path.join(videoDir, 'e2e_recording.webm');
            fs.renameSync(oldPath, newPath);
            log(`📹 Video saved: e2e_recording.webm`);
        }
    }
})();
