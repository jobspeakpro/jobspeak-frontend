...
export default function DevDebugBanner() {
    ...
    return (
        <div className="fixed bottom-0 right-0 z-[9999] bg-black/80 text-white px-3 py-2 text-xs font-mono rounded-tl-lg pointer-events-auto opacity-50 hover:opacity-100 transition-opacity flex gap-4 items-center">
            <span>API: {isRelative ? "[Relative]" : API_BASE} | Mode: {import.meta.env.MODE}</span>
            <button
                onClick={() => {
